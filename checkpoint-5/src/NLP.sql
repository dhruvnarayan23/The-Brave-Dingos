...
-- drop view if exists officer_css_awards cascade;
-- create TEMP view officer_css_awards as (
-- select A.officer_id, round(avg(A.CSS),4) as CSS, count(B.award_type) as award_count
-- from officer_css A left join data_award B on A.officer_id = B.officer_id
-- group by A.officer_id
-- );

-- select * from officer_css_awards;
-- select count(officer_id), count(distinct officer_id), max(CSS), min(CSS), min(award_count), max(award_count) from officer_css_awards;

drop view if exists officer_allegation_summary cascade;
create TEMP view officer_allegation_summary as (
select A.summary, B.officer_id
from data_allegation A inner join data_officerallegation B on A.crid = B.allegation_id
where A.summary is not null and A.summary != ''
);

drop view if exists officer_css_summary cascade;
create TEMP view officer_css_summary as (
select A.officer_id, A.CSS, B.summary from officer_css A
inner join officer_allegation_summary B on A.officer_id = B.officer_id
);

select * from officer_css_summary;